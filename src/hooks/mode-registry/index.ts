 ...
 *
 * All modes store state in `.omd/state/` subdirectory for consistency.
 */
 ...
 * All paths are relative to .omd/state/ directory.
 */
    ...
    for (const exclusiveMode of EXCLUSIVE_MODES) {
      if (exclusiveMode !== mode && isModeActive(exclusiveMode, cwd)) {
        ...
        return {
          ...
          message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /oh-my-droid:cancel.`
        };
      }
    }
  }
