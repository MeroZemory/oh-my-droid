...
function getStateFilePath(directory: string): string {
  ...
  return join(omdDir, 'state', STATE_FILE);
}
...
export function ensureAutopilotDir(directory: string): string {
  ...
  const autopilotDir = join(directory, '.omd', SPEC_DIR);
...
export function getSpecPath(directory: string): string {
  return join(directory, '.omd', SPEC_DIR, 'spec.md');
}
...
export function getPlanPath(directory: string): string {
  return join(directory, '.omd', 'plans', 'autopilot-impl.md');
}
