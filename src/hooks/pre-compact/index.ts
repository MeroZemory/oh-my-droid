...
export interface HookOutput {
  continue: boolean;
  /** System message for context injection (Factory Droid compatible) */
...
export function getCheckpointPath(directory: string): string {
  const checkpointDir = join(directory, '.omd', 'state', CHECKPOINT_DIR);
...
export async function exportWisdomToNotepad(directory: string): Promise<{ wisdom: string; exported: boolean }> {
  const notepadsDir = join(directory, '.omd', 'notepads');
