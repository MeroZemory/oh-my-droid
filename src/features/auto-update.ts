...
export function formatUpdateNotification(checkResult: UpdateCheckResult): string {
  ...
  const lines = [
    ...
    '  Or run: curl -fsSL https://raw.githubusercontent.com/Yeachan-Heo/oh-my-droid/main/scripts/install.sh | bash',
    ''
  ];
...
const SILENT_UPDATE_STATE_FILE = join(FACTORY_CONFIG_DIR, '.omd-silent-update.json');
...
export async function silentAutoUpdate(config: SilentUpdateConfig = {}): Promise<UpdateResult | null> {
  const {
    ...
    autoApply = true,
    logFile = join(FACTORY_CONFIG_DIR, '.omd-update.log'),
    maxRetries = 3
  } = config;
