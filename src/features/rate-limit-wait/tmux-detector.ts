 ...
 * tmux Detector
 *
 * Detects Factory Droid sessions running in tmux panes and identifies
...
/**
 * Analyze pane content to determine if it shows a rate-limited Factory Droid session
 */
...
/**
 * Scan all tmux panes for blocked Factory Droid sessions
 *
...
export function formatBlockedPanesSummary(blockedPanes: BlockedPane[]): string {
  if (blockedPanes.length === 0) {
    return 'No blocked Factory Droid sessions detected.';
  }

  const lines: string[] = [
    `Found ${blockedPanes.length} blocked Factory Droid session(s):`,
    '',
  ];
