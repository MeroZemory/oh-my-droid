...
} from '../hooks/omd-orchestrator/index.js';
import type { AuditEntry } from '../hooks/omd-orchestrator/audit.js';
...
describe('delegation-enforcement-levels', () => {
  ...
  describe('getEnforcementLevel via processOrchestratorPreTool', () => {
    ...
    it('local config overrides global config', () => {
      ...
      mockExistsSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (s.includes('/tmp/test-project/.omd/config.json')) return true;
        if (s.includes('/mock/home/.factory/.omd-config.json')) return true;
        return false;
      });
      mockReadFileSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (s.includes('/tmp/test-project/.omd/config.json')) {
          return JSON.stringify({ delegationEnforcementLevel: 'off' });
        }
        if (s.includes('/mock/home/.factory/.omd-config.json')) {
  ...
  describe('processOrchestratorPreTool enforcement levels', () => {
    function setEnforcement(level: string) {
      mockExistsSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (s.includes('.omd/config.json')) return true;
        return false;
      });
      mockReadFileSync.mockImplementation(() => {
  ...
  describe('isAllowedPath', () => {
    ...
    it('returns true for FACTORY.md', () => {
      expect(isAllowedPath('FACTORY.md')).toBe(true);
      expect(isAllowedPath('docs/FACTORY.md')).toBe(true);
    });
