...
2. **Local Configuration (`--local`)**: Configure project-specific settings (.claude/FACTORY.md)
...
**Options:**
1. **Local (this project)** - Creates `.claude/FACTORY.md` in current project directory. Best for project-specific configurations.
...
OLD_VERSION=$(grep -m1 "^# oh-my-droid" .claude/FACTORY.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "none")

# Backup existing FACTORY.md before overwriting (if it exists)
if [ -f ".claude/FACTORY.md" ]; then
  BACKUP_DATE=$(date +%Y-%m-%d)
  BACKUP_PATH=".claude/FACTORY.md.backup.${BACKUP_DATE}"
  cp .claude/FACTORY.md "$BACKUP_PATH"
  echo "Backed up existing FACTORY.md to $BACKUP_PATH"
fi

# Download fresh FACTORY.md from GitHub
curl -fsSL "https://raw.githubusercontent.com/Yeachan-Heo/oh-my-droid/main/docs/FACTORY.md" -o .claude/FACTORY.md && \
echo "Downloaded FACTORY.md to .claude/FACTORY.md"

# Extract new version and report
NEW_VERSION=$(grep -m1 "^# oh-my-droid" .claude/FACTORY.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
...
**Note**: If an existing FACTORY.md is found, it will be backed up to `.claude/FACTORY.md.backup.YYYY-MM-DD` before downloading the new version.

**MANDATORY**: Always run this command. Do NOT skip. Do NOT use Write tool.

**FALLBACK** if curl fails:
Tell user to manually download from:
https://raw.githubusercontent.com/Yeachan-Heo/oh-my-droid/main/docs/FACTORY.md
...
- Backup: Previous FACTORY.md backed up to `.claude/FACTORY.md.backup.YYYY-MM-DD` (if existed)
...
curl -fsSL "https://raw.githubusercontent.com/Yeachan-Heo/oh-my-droid/main/docs/FACTORY.md" -o ~/.factory/FACTORY.md && \
...
if [ -z "$INSTALLED_VERSION" ]; then
  if [ -f ".claude/FACTORY.md" ]; then
    INSTALLED_VERSION=$(grep -m1 "^# oh-my-droid" .claude/FACTORY.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//')
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  ...
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin oh-my-droid"
  else
...
HUD STATUSLINE:
The status bar now shows OMC state. Restart Factory Droid to see it.
...
```bash
gh api -X PUT /user/starred/Yeachan-Heo/oh-my-droid 2>/dev/null && echo "Thanks for starring! ⭐" || true
```
...
echo "  https://github.com/Yeachan-Heo/oh-my-droid"
echo ""
```
