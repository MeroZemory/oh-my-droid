...
PLUGIN_VERSION=$(ls ~/.factory/plugins/cache/omc/oh-my-droid/ 2>/dev/null | sort -V | tail -1)
if [ -n "$PLUGIN_VERSION" ]; then
  ls ~/.factory/plugins/cache/omc/oh-my-droid/$PLUGIN_VERSION/dist/hud/index.js 2>/dev/null && echo "BUILT" || echo "NOT_BUILT"
fi
```
...
```bash
cd ~/.factory/plugins/cache/omc/oh-my-droid/$PLUGIN_VERSION && npm install
```
...
async function main() {
  ...
  const pluginCacheBase = join(home, ".claude/plugins/cache/omc/oh-my-droid");
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.claude','hud','omd-hud.mjs');console.log(JSON.stringify(p))"
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:\\Users\\username\\.claude\\hud\\omd-hud.mjs"
  }
}
```
