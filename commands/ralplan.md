...
Ralplan maintains persistent state in `.omd/ralplan-state.json` to track progress and enable recovery across interruptions:
...
1. Create `.omd/plans/` directory if it doesn't exist
...
- Planner generates plan directly to `.omd/plans/[feature-name].md`
...
- Signal completion with `PLAN_READY: .omd/plans/[filename].md`
